...
//
// Use Extractor for schema generation and validation without a full Tool pipeline (e.g. in custom orchestrators). Use NewDynamicTool when the schema is only available at runtime (e.g. from OpenAPI); it validates against the schema and passes raw JSON to the handler.
